...
no_filtering_stats = {"N_and_O": 0, "neither": 0, "N_only": 0, "O_only": 0}
...
print(
    f"Kept N-only and O-only: {no_filtering_stats['N_only'] + no_filtering_stats['O_only']}"
)
...
median_ehull_by_group = (
    df.groupby("metal_oxi_no_combo")
    .agg(
        {
            "ehull": "median",
            "metal_type": "first",
...
def get_ionic_radius(metal_type, oxidation_state):
    ...
    return element.ionic_radii.get(oxidation_state, None)
...
plt.ylabel(r"Median Δ$E_{\mathrm{hull}}$ (eV/atom)", fontsize=22)


# Legend for HSAB classification (colors)
hsab_legend_elements = [
    Line2D(
        [0],
        [0],
        marker="o",
        color="w",
        markerfacecolor=hsab_colors[hsab_class],
        markersize=10,
        markeredgecolor="black",
        markeredgewidth=1,
        label=hsab_class,
    )
    for hsab_class in sorted(hsab_colors.keys())
]
...
ligand_legend_elements = [
    ...
    Line2D(
        ...
        markeredgecolor="black",
        markeredgewidth=1,
        label="N-only ligands",
    ),
]

# Add both legends
hsab_legend = ax.legend(
    handles=hsab_legend_elements,
    title="Metal HSAB Classification",
    loc="upper right",
    frameon=False,
    fontsize=20,
    title_fontsize=20,
...
ligand_legend = ax.legend(
    handles=ligand_legend_elements,
    title="Ligand Type",
...
print("=" * 130)
for _idx, row in median_ehull_by_group.iterrows():
    ligand_type = "O-only" if row["no_category"] == "O_only" else "N-only"
    marker = "○" if row["no_category"] == "O_only" else "△"
    print(
        f"{row['metal_type']}(+{row['oxidation_state']}) {ligand_type} {marker}: ehull={row['ehull']:.4f}, ionic_radius={row['ionic_radius']:.3f}, count={row['count']}, HSAB={row['hsab_class']}"
    )
...
for hsab_class in ["Hard", "Borderline", "Soft", "Unknown"]:
    ...
    if not subset.empty:
        ...
        print(
            f"  {hsab_class} acids: {len(subset)} groups, avg ehull = {avg_ehull:.4f} eV/atom"
        )
