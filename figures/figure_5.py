...
print(
    f"Percentage with data: {df['pore_diameters'].notna().sum() / len(df) * 100:.1f}%"
)
...
color_map = {True: "black", False: "black"}
...
ax2.set_ylabel(r"$ΔE_{\mathrm{form}}$ (eV/atom)", fontsize=10)
...
ax1.scatter(
    df["pore_diameters"], df["ehull"], c=colors_all, s=4, alpha=0.3, edgecolors="none"
)
...
plt.tight_layout()
plt.savefig("Figure5.png", dpi=1500, bbox_inches="tight")
plt.show()
