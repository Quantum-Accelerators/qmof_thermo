...
df = pd.DataFrame.from_dict(data, orient="index").rename_axis("qmof_id").reset_index()
...
plt.title("MP Materials", fontsize=10)
ax.set_ylabel(r"$ΔE_{\mathrm{form}}$ (eV/atom)", fontsize=8)
ax.set_xlim([0, 1.02])
ax.set_ylim([-3, 0.1])
...
plt.tight_layout()
plt.savefig("FigureS23a.png", bbox_inches="tight", dpi=1500)
plt.show()
