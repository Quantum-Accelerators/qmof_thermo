...
patterns_with_matches = sum(1 for entries in pattern_to_entries.values() if len(entries) > 1)
total_matched_entries = sum(len(entries) for entries in pattern_to_entries.values() if len(entries) > 1)
...
ax.set_xlabel("Δ$E_{\mathrm{hull}}$ Range (eV/atom)", fontsize=16)
ax.set_ylabel("Number of Polymorph Groups", fontsize=16)
ax.set_title("Distribution of Δ$E_{\mathrm{hull}}$ Ranges for Polymorph Groups", fontsize=16)
