...
mp_syn_ehull = mp_df[mp_df["theoretical"]]["energy_above_hull"].to_numpy()

mp_nonsyn_ehull = mp_df[not mp_df["theoretical"]]["energy_above_hull"].to_numpy()

mp_syn_eform = mp_df[mp_df["theoretical"]]["formation_energy_per_atom"].to_numpy()

mp_nonsyn_eform = mp_df[not mp_df["theoretical"]][
...
df_qmof = df_qmof[
    (df_qmof.formation_energy >= -1.3)
    & (df_qmof.formation_energy <= 0.4)
    & (df_qmof.ehull >= 0)
    & (df_qmof.ehull <= 1.5)
]

hb1 = ax1.hexbin(
    df_qmof["formation_energy"], df_qmof["ehull"], gridsize=80, cmap="viridis", mincnt=1
)
...
ax1.set_xlabel(r"$ΔE_{\mathrm{form}}$ (eV/atom)", fontsize=10)
ax1.set_ylabel(r"$ΔE_{\mathrm{hull}}$ (eV/atom)", fontsize=10)
...
hb2 = ax2.hexbin(df_mp["eform"], df_mp["ehull"], gridsize=30, cmap="viridis", mincnt=1)
...
ax2.set_xlabel(r"$ΔE_{\mathrm{form}}$ (eV/atom)", fontsize=10)
ax2.set_ylabel(r"$ΔE_{\mathrm{hull}}$ (eV/atom)", fontsize=10)
...
qmof_df_syn_c = qmof_df_c[qmof_df_c["synthesizable"]][["formation_energy"]].assign(
    Dataset="All MOFs in QMOF (Synthesized)"
)
qmof_df_nonsyn_c = qmof_df_c[not qmof_df_c["synthesizable"]][
    ["formation_energy"]
].assign(Dataset="All MOFs in QMOF (Hypothetical)")

df_c = pd.concat(
    [mp_df_syn_c, mp_df_nonsyn_c, qmof_df_syn_c, qmof_df_nonsyn_c], ignore_index=True
)
...
sns.histplot(
    data=df_c[df_c.Dataset == "MP within QMOF Chemsys (Synthesized)"],
    x="formation_energy",
    bins=bin_edges_c,
    stat="probability",
    element="step",
    ...
    alpha=1,
    linewidth=1.25,
    label="MP within QMOF Chemsys (Synthesized)",
    ax=ax3,
)

sns.histplot(
    data=df_c[df_c.Dataset == "All MOFs in QMOF (Synthesized)"],
    x="formation_energy",
    bins=bin_edges_c,
    stat="probability",
    element="step",
    ...
    alpha=1,
    linewidth=1.25,
    label="All MOFs in QMOF (Synthesized)",
    ax=ax3,
)

sns.histplot(
    data=df_c[df_c.Dataset == "All MOFs in QMOF (Hypothetical)"],
    x="formation_energy",
    bins=bin_edges_c,
    stat="probability",
    element="step",
    ...
    alpha=1,
    linewidth=1.25,
    label="All MOFs in QMOF (Unsynthesized)",
    ax=ax3,
)

sns.histplot(
    data=df_c[df_c.Dataset == "MP within QMOF Chemsys (Hypothetical)"],
    x="formation_energy",
    bins=bin_edges_c,
    stat="probability",
    element="step",
    ...
    alpha=1,
    linewidth=1.25,
    label="MP within QMOF Chemsys (Unsynthesized)",
    ax=ax3,
)
...
ax3.set_xlabel(r"$ΔE_{\mathrm{form}}$ (eV/atom)", fontsize=10)
...
df_d = pd.concat(
    [mp_df_syn_d, mp_df_nonsyn_d, qmof_df_syn_d, qmof_df_nonsyn_d], ignore_index=True
)
...
sns.histplot(
    data=df_d[df_d.Dataset == "MP within QMOF Chemsys (Synthesized)"],
    x="ehull",
    bins=bin_edges_d,
    stat="probability",
    element="step",
    ...
    color="#3CB371",
    linewidth=1.25,
    label="MP - Synthesized",
    ax=ax4,
)

sns.histplot(
    data=df_d[df_d.Dataset == "MP within QMOF Chemsys (Hypothetical)"],
    x="ehull",
    bins=bin_edges_d,
    stat="probability",
    element="step",
    ...
    color="#FF8C00",
    linewidth=1.25,
    label="MP - Hypothetical",
    ax=ax4,
)

sns.histplot(
    data=df_d[df_d.Dataset == "All MOFs in QMOF (Synthesized)"],
    x="ehull",
    bins=bin_edges_d,
    stat="probability",
    element="step",
    ...
    color="#6495ED",
    linewidth=1.25,
    label="QMOF - Synthesized",
    ax=ax4,
)

sns.histplot(
    data=df_d[df_d.Dataset == "All MOFs in QMOF (Hypothetical)"],
    x="ehull",
    bins=bin_edges_d,
    stat="probability",
    element="step",
...
ax4.set_xlabel(r"$ΔE_{\mathrm{hull}}$ (eV/atom)", fontsize=10)
ax4.set_ylabel("Relative Frequency", fontsize=10)
ax4.set_xlim([x_min_d, x_max_d])
ax4.set_ylim([0, 0.56])

ax4.legend(
    title="Dataset", fontsize=8, title_fontsize=8, frameon=False, loc="upper right"
)
