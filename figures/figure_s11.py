...
threshold_values = [optimal_ehull, 0.267, 0.306, 0.353, 0.476]  # Ehull values you care about
...
for thresh, color in zip(threshold_values, colors):
    ...
    # Plot point
    plt.plot(fpr[idx], tpr[idx], 'o', color=color, markersize=11, 
             label= 'Δ$E_{\mathrm{hull}}$ = '+str(actual_val)+' eV/atom')
    
    # Add text label next to point
    plt.text(fpr[idx], tpr[idx]-0.1, 
             f'{actual_val:.3f}\n(TPR={tpr[idx]:.2f})', 
...
plt.title('ROC Curve with Δ$E_{\mathrm{hull}}$ Thresholds', fontsize=16)
