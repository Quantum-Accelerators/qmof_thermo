...
hsab_colors = {"Hard": "#1f77b4", "Soft": "#d62728", "Borderline": "#9467bd"}
...
for qmof_id, qmof_data in data.items():
    ...
    is_valid, metal_type, oxidation_state, reason = analyze_oxidation_states(
        qmof_data.get("oxidation_states")
    )
...
print(
    f"Synthesized MOFs: {synthesized_count} ({synthesized_count / total_mofs * 100:.1f}%)"
)
print("\nFiltering breakdown (synthesized only):")
print(
    f"  ✓ Valid for plotting: {filtering_stats['valid']} ({filtering_stats['valid'] / synthesized_count * 100:.1f}%)"
)
print("\nExcluded:")
print(
    f"  No oximachine data: {filtering_stats['no_oximachine_data']} ({filtering_stats['no_oximachine_data'] / synthesized_count * 100:.1f}%)"
)
print(
    f"  Incomplete oximachine data: {filtering_stats['incomplete_oximachine_data']} ({filtering_stats['incomplete_oximachine_data'] / synthesized_count * 100:.1f}%)"
)
print(
    f"  Multiple metal types: {filtering_stats['multiple_metal_types']} ({filtering_stats['multiple_metal_types'] / synthesized_count * 100:.1f}%)"
)
print(
    f"  Disagreeing oxidation states: {filtering_stats['disagreeing_oxidation_states']} ({filtering_stats['disagreeing_oxidation_states'] / synthesized_count * 100:.1f}%)"
)
print(
    f"  Low confidence (<85%): {filtering_stats['low_confidence']} ({filtering_stats['low_confidence'] / synthesized_count * 100:.1f}%)"
)
print(
    f"  Not N-only or O-only: {filtering_stats['not_N_or_O_only']} ({filtering_stats['not_N_or_O_only'] / synthesized_count * 100:.1f}%)"
)
print(
    f"  Unknown HSAB classification: {filtering_stats['unknown_hsab']} ({filtering_stats['unknown_hsab'] / synthesized_count * 100:.1f}%)"
)
...
if unknown_hsab:
    ...
    print("Metal-Oxidation combinations not in hsab_simple dictionary:")
...
df_c = df_c[df_c["synthesizable"]].copy()
...
df_b["group"] = pd.Categorical(df_b["group"], categories=group_order, ordered=True)
...
palette = dict(zip(group_order, list(red_gradient) + list(blue_gradient), strict=False))
...
labels_with_counts = [
    f"{name}\n({group_counts.get(name_mapping[name], 0)})" for name in group_names
]
...
if len(sorted_categories) > 0:
    ...
    for y in ax2.get_yticks():
        ax2.axhline(
            y=y, linestyle="-", linewidth=1.05, color="grey", alpha=0.8, zorder=0
        )
