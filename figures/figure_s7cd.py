...
print(f"Above filter: {len(differences_above)} ({len(differences_above)/len(differences)*100:.1f}%)")
...
if len(differences_above) > 0:
    ...
    outliers = [(qmof_ids[i], differences[i]) for i in range(len(differences)) if differences[i] > filter_val]
...
if len(differences_below) > 0:
    ...
    n1, bins1, patches1 = ax1.hist(differences_below, bins=10, weights=weights1,
                                    color='#6495ED', edgecolor='black', alpha=0.7)
    ax1.set_xlabel('Nonzero ΔΔ$E_{\mathrm{hull}}$ (eV/atom)\nMP$_{\mathrm{within}}$ $_{0.005}$ $_{\mathrm{eV/atom}}$ -> MP$_{\mathrm{within}}$ $_{0.010}$ $_{\mathrm{eV/atom}}$', fontsize=14)
    ax1.set_ylabel('Normalized Frequency', fontsize=14)
    ax1.set_title('Main Distribution of nonzero ΔΔ$E_{\mathrm{hull}}$'+f' (≤ {filter_val} eV/atom)\n$n$ = {len(differences_below)}', fontsize=14)
    ...
    ax1.text(0.95, 0.95, textstr, transform=ax1.transAxes, fontsize=14,
...
if len(differences_above) > 0:
    ...
    n2, bins2, patches2 = ax2.hist(differences_above, bins=10, weights=weights2,
                                    color='#FF6347', edgecolor='black', alpha=0.7)
    ax2.set_xlabel('Nonzero ΔΔ$E_{\mathrm{hull}}$ (eV/atom)\nMP$_{\mathrm{within}}$ $_{0.005}$ $_{\mathrm{eV/atom}}$ -> MP$_{\mathrm{within}}$ $_{0.010}$ $_{\mathrm{eV/atom}}$', fontsize=14)
    ax2.set_ylabel('Normalized Frequency', fontsize=14)
    ax2.set_title('Outliers of nonzero ΔΔ$E_{\mathrm{hull}}$'+f'(> {filter_val} eV/atom)\n$n$ = {len(differences_above)}', fontsize=14)
...
plt.tight_layout()
plt.savefig("FigureS7cd.png", dpi=300, bbox_inches='tight')
plt.show()
...
print(f"  Positive (ehull increased): {positive_changes} ({positive_changes/len(differences)*100:.1f}%)")
print(f"  Negative (ehull decreased): {negative_changes} ({negative_changes/len(differences)*100:.1f}%)")
