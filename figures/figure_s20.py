...
df = df[["qmof_id", "ehull", "linker_types", "synthesizable", "chemsys", "monometallic_cation"]]
...
df_filtered[['metal_element', 'oxidation_state']] = df_filtered['monometallic_cation'].apply(
    lambda x: pd.Series(extract_metal_info(x))
)
...
print("="*60)
sample_df = df_filtered[['qmof_id', 'metal_element', 'oxidation_state', 'hsab_class']].head(20)
for idx, row in sample_df.iterrows():
    key = f"{row['metal_element']}_{int(row['oxidation_state'])}"
    print(f"{row['qmof_id']}: {row['metal_element']} with ox state {row['oxidation_state']} -> key: {key} -> {row['hsab_class']}")
...
hsab_counts = df_filtered.groupby(['linker_category', 'hsab_class']).size().reset_index(name='count')

# Calculate total MOFs per linker category
category_totals = df_filtered.groupby('linker_category').size().reset_index(name='total')
...
# Add legend
ax.legend(title="HSAB Class", loc='upper right',bbox_to_anchor=(1.3, 1.05), 
          fontsize=16, title_fontsize=16, frameon=False)
...
for category in sorted_categories:
    ...
    cat_data_copy['metal_ion'] = cat_data_copy.apply(
        lambda row: f"{row['metal_element']}({int(row['oxidation_state'])}+)", 
        axis=1
    )
