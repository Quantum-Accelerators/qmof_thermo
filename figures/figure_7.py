...
print(f"Carbon Fraction vs Formation Energy: ρ = {corr_eform:.4f}, p-value = {pval_eform}")
print(f"Carbon Fraction vs Energy Above Hull: ρ = {corr_ehull:.4f}, p-value = {pval_ehull}")
print(f"Sample size: {len(df)}")

# Get shared colorbar range
fig_temp, (ax1_temp, ax2_temp) = plt.subplots(1, 2)
hb1_temp = ax1_temp.hexbin(df["molfrac"], df["formation_energy"], gridsize=80, mincnt=1, 
                            extent=[0, 0.7, -2.3, 0.7])
hb2_temp = ax2_temp.hexbin(df["molfrac"], df["ehull"], gridsize=80, mincnt=1, 
...
gs = gridspec.GridSpec(2, 14, figure=fig, height_ratios=[1, 1],
...
ax1.text(-0.5, 1, "A", transform=ax1.transAxes, fontsize=11,
         fontweight="bold", va="top", ha="left")

# ============== PLOT B: Formation Energy vs Carbon Fraction ==============
ax2 = fig.add_subplot(gs[1, 0:6])  # Columns 0 and 1

hb2 = ax2.hexbin(df["molfrac"], df["formation_energy"],
                 gridsize=80, cmap='viridis', mincnt=1,
                 extent=[0, 0.7, -2.3, 0.7],
                 norm=LogNorm(vmin=vmin, vmax=vmax))

ax2.text(0.95, 0.2, f'$ρ$ = {corr_eform:.3f}',
...
ax2.set_ylabel("$ΔE_{\mathrm{form}}$ (eV/atom)", fontsize=8)

ax2.text(-0.2, 1.00, "B", transform=ax2.transAxes, fontsize=11,
         fontweight="bold", va="top", ha="left")

# ============== PLOT C: Ehull vs Carbon Fraction ==============
ax3 = fig.add_subplot(gs[1, 7:14])  # Columns 2 and 3

hb3 = ax3.hexbin(df["molfrac"], df["ehull"],
                 gridsize=80, cmap='viridis', mincnt=1,
                 extent=[0, 0.7, -0.1, 0.85],
                 norm=LogNorm(vmin=vmin, vmax=vmax))

ax3.text(0.95, 0.95, f'$ρ$ = {corr_ehull:.3f}',
...
ax3.set_ylabel("$ΔE_{\mathrm{hull}}$ (eV/atom)", fontsize=8)

ax3.text(-0.17, 1.0, "C", transform=ax3.transAxes, fontsize=11,
         fontweight="bold", va="top", ha="left")
