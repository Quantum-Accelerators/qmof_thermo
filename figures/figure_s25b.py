...
df_nonsynth = df[not df["synthesizable"]]

fig, ax = plt.subplots(figsize=(8, 6))

ax.scatter(
    df_synth["molfrac"],
    df_synth["pore_diameters"],
    c="#6495ED",
    alpha=1,
    s=20,
    label=f"Synthesized ({len(df_synth)})",
)

# Plot non-synthesized first (background)
ax.scatter(
    df_nonsynth["molfrac"],
    df_nonsynth["pore_diameters"],
    facecolors="none",
    edgecolors="#FF6347",
    alpha=1,
    s=20,
    linewidths=0.4,
    label=f"Not Synthesized ({len(df_nonsynth)})",
)
