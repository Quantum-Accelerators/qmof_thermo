...
for properties in data2.values():
...
df["source"] = pd.Categorical(df["source"], categories=filter_list, ordered=True)

blue_shades = sns.color_palette("Blues", 3)
red_shades = sns.color_palette("Reds", 6)
green_shades = sns.color_palette("Greens", 3)
palette = dict(zip(filter_list, blue_shades + red_shades + green_shades, strict=False))
...
new_labels = [f"{src} ({counts[src]})" for src in filter_list]
